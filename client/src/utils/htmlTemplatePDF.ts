...
// Daily PDF configuration optimized for 8.5x11 portrait with proper measurements
const REMARKABLE_DAILY_CONFIG = {
  // Extended portrait format for full timeline + footer
  pageWidth: 612,   // 8.5 inches
  pageHeight: 1000, // Extended height for full timeline + footer visibility
  margin: 30,       // Standard margin

  // Header configuration - proper proportions
  headerHeight: 88,  // Matches audit expectations
  statsHeight: 42,   
  legendHeight: 30,  
  ...
  // Grid configuration - exact measurements from audit
  timeColumnWidth: 70,  // Matches audit expectations

  get timeSlotHeight() {
    // Fixed 40px per slot as expected by audit
    return 40;
  },
  ...
  // Typography - optimized hierarchy from audit
  fonts: {
    title: 20,         // 20px bold from audit
    subtitle: 14,      
    stats: 14,         // 14px medium from audit
    timeSlot: 13,      // 13px for hours from audit
    timeSlotHalf: 11,  // 11px for half-hours from audit
    eventTitle: 10,    // 10px for appointment title from audit
    eventSource: 9,    // 9px for source from audit
    eventTime: 9       // 9px for time from audit
  },
  ...
  colors: {
    ...
    mediumGray: [229, 231, 235], // #e5e7eb
    darkGray: [107, 114, 128],   // #6b7280
    simplePracticeBlue: [99, 102, 241], // #6366f1 from audit
    googleGreen: [5, 150, 105],  // #059669 from audit
    holidayYellow: [217, 119, 6] // #d97706 from audit
  }
};
...
// Time slots from 06:00 to 23:30 (matches HTML template) - FIXED to include all slots
const TIME_SLOTS = [
...
export function drawDailyFooter(pdf: jsPDF, selectedDate: Date, pageNumber: number = 1, dayOfWeek: number = 1): void {
  ...
  // Footer area - positioned to ensure full timeline visibility
  const footerY = pageHeight - margin - 60; // More space from bottom
  const footerHeight = 50; // Slightly taller footer
  ...
  pdf.rect(margin + 20, buttonY, buttonWidth, buttonHeight, 'FD');
  pdf.setFontSize(9);
  pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.black);
  pdf.text('← Previous Day', margin + 20 + buttonWidth/2, buttonY + 16, { align: 'center' });

  // Weekly Overview button
  pdf.rect(pageWidth/2 - buttonWidth/2, buttonY, buttonWidth, buttonHeight, 'FD');
  pdf.text('📅 Weekly View', pageWidth/2, buttonY + 16, { align: 'center' });

  // Next Day button
  pdf.rect(pageWidth - margin - 20 - buttonWidth, buttonY, buttonWidth, buttonHeight, 'FD');
  pdf.text('Next Day →', pageWidth - margin - 20 - buttonWidth/2, buttonY + 16, { align: 'center' });
  ...
  pdf.text('Return to Weekly Overview (Page 1)', margin + 10, footerY + 16);

  // Center: Current page info with bidirectional context
  const centerText = `${currentDayName} - Page ${pageNumber} of 8`;
  pdf.text(centerText, pageWidth / 2, footerY + 16, { align: 'center' });
  ...
  pdf.text(navText, pageWidth - margin - 10, footerY + 16, { align: 'right' });

  // Additional navigation helper
  pdf.setFontSize(7);
  pdf.setTextColor(100, 100, 100);
  pdf.text('All pages are bidirectionally linked', pageWidth / 2, footerY + 32, { align: 'center' });

  // Timeline completion indicator
  pdf.setFontSize(8);
  pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.darkGray);
  pdf.text('Timeline: 06:00 - 23:30 (Complete)', pageWidth / 2, footerY + 42, { align: 'center' });
}
...
export function drawDailyGrid(pdf: jsPDF, selectedDate: Date, events: CalendarEvent[]): void {
  ...
  // Generate time slots exactly like the daily view (6:00-23:30) - ALL 36 SLOTS
  const timeSlots = [];
  for (let hour = 6; hour <= 23; hour++) {
    ...
    // Add half-hour slot (including 23:30)
    if (hour < 23 || hour === 23) { // Ensure 23:30 is included
      timeSlots.push({
  ...
  timeSlots.forEach((slot, index) => {
    ...
    // Time column cell with proper alternating colors
    if (slot.isHour) {
      pdf.setFillColor(...REMARKABLE_DAILY_CONFIG.colors.lightGray);
    } else {
      pdf.setFillColor(255, 255, 255); // White for half-hour slots
    }
    pdf.rect(margin, y, timeColumnWidth, timeSlotHeight, 'F');

    // Time text with proper font sizes - ALWAYS VISIBLE
    pdf.setFontSize(slot.isHour ? REMARKABLE_DAILY_CONFIG.fonts.timeSlot : REMARKABLE_DAILY_CONFIG.fonts.timeSlotHalf);
    pdf.setFont('helvetica', slot.isHour ? 'bold' : 'normal');
    pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.black); // Changed to black for better visibility
    pdf.text(slot.time, margin + timeColumnWidth / 2, y + timeSlotHeight / 2 + 4, { align: 'center' });

    // Day cell
    pdf.setFillColor(...REMARKABLE_DAILY_CONFIG.colors.white);
    pdf.rect(dayX, y, dayColumnWidth, timeSlotHeight, 'F');

    // Horizontal grid lines - ALL VISIBLE
    pdf.setLineWidth(slot.isHour ? 1.5 : 0.5);
    pdf.setDrawColor(...REMARKABLE_DAILY_CONFIG.colors.mediumGray);
    pdf.line(margin, y + timeSlotHeight, margin + timeColumnWidth + dayColumnWidth, y + timeSlotHeight);
  });

  // === VERTICAL GRID LINES ===
  // Main time column separator
  pdf.setLineWidth(2);
  pdf.setDrawColor(...REMARKABLE_DAILY_CONFIG.colors.black);
  ...
  // Outer border to match daily view
  pdf.setLineWidth(2);
  pdf.setDrawColor(...REMARKABLE_DAILY_CONFIG.colors.black);
  pdf.rect(margin, gridY, timeColumnWidth + dayColumnWidth, headerHeight + totalGridHeight);
...
function drawRemarkableDailyAppointments(pdf: jsPDF, selectedDate: Date, events: CalendarEvent[], gridStartY: number, dayColumnWidth: number, timeSlotHeight: number): void {
  ...
  const dayEvents = events.filter(event => {
    ...
    // CRITICAL: Filter out test appointments and ghost events
    if (event.title && (
  ...
  dayEvents.forEach((event, index) => {
    ...
    // Height calculation - ensure proper spacing for all text elements
    let eventHeight = Math.max(80, (durationMinutes / 30) * timeSlotHeight - 4); // Increased minimum to 80px for better text spacing
    ...
    if (needsExpandedLayout) {
      ...
      const minimumHeight = 90 + (maxContentLines * 14); // Increased space for better layout
    ...
    // === IMPROVED TEXT RENDERING WITH AUDIT-COMPLIANT FONTS ===
    const padding = isSimplePractice ? 6 : 4;
    ...
    if (needsExpandedLayout) {
      ...
      const col3X = startX + col1Width + col2Width + 12;

      // Draw column dividers
      pdf.setDrawColor(...REMARKABLE_DAILY_CONFIG.colors.mediumGray);
      pdf.setLineWidth(0.5);
      pdf.line(col2X - 3, eventY + 6, col2X - 3, eventY + eventHeight - 6);
      pdf.line(col3X - 3, eventY + 6, col3X - 3, eventY + eventHeight - 6);

      // === COLUMN 1: Event Info ===
      let col1Y = eventY + 14;

      // Event title - using audit-compliant font size
      pdf.setFontSize(REMARKABLE_DAILY_CONFIG.fonts.eventTitle);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.black);

      // Simple text rendering
      const titleMaxWidth = col1Width - 4;
      const titleLines = pdf.splitTextToSize(displayTitle, titleMaxWidth);
      ...
      // Source with audit-compliant font
      if (col1Y + 10 <= eventY + eventHeight - 16) {
      ...
      // Military time with audit-compliant font
      if (col1Y + 10 <= eventY + eventHeight - 6) {
        pdf.setFontSize(REMARKABLE_DAILY_CONFIG.fonts.eventTime);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.black);

        // Military time format as required by audit
      ...
      // === SIMPLE LAYOUT (No notes/action items) - Audit Compliant ===
      let currentY = eventY + 12;

      // Event title with better spacing
      pdf.setFontSize(REMARKABLE_DAILY_CONFIG.fonts.eventTitle);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.black);
      ...
      if (displayTitle && displayTitle.length > 0) {
        ...
        for (let i = 0; i < Math.min(titleLines.length, 2); i++) {
          if (currentY + 12 <= eventY + eventHeight - 30) {
      ...
      // Source with better spacing
      if (currentY + 10 <= eventY + eventHeight - 20) {
        pdf.setFontSize(REMARKABLE_DAILY_CONFIG.fonts.eventSource);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(...REMARKABLE_DAILY_CONFIG.colors.darkGray);
      ...
      // Add spacing between source and time
      currentY += 2;

      // Military time with proper bottom margin to prevent cutoff
...
function drawDailyAppointments(pdf: jsPDF, selectedDate: Date, events: CalendarEvent[], gridStartY: number, dayColumnWidth: number): void {
  ...
  const dayEvents = events.filter(event => {
    const eventDate = new Date(event.startTime);
    const dateMatches = eventDate.toDateString() === selectedDate.toDateString();
